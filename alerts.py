...
def send_telegram(signals, snapshot, executed, startup=False):
    ...
    for sig in signals:
        ...
        chg_1h  = snapshot.get(ticker, {}).get("change_1h", 0)
        chg_str = ("+" if chg >= 0 else "") + str(round(chg, 2)) + "%"
        h_str   = ("+" if chg_1h >= 0 else "") + str(round(chg_1h, 2)) + "%"
        icon    = "+" if action == "BUY" else ("-" if action == "SELL" else "~")
        lines.append(icon + " <b>" + ticker + "</b> " + action + " " + str(conf) + "% | " + tf + " | 1h:" + h_str + " 24h:" + chg_str)
    lines.append("")
    if executed:
        stop_losses = [t for t in executed if "STOP LOSS" in t.get("reason", "")]
        normal      = [t for t in executed if "STOP LOSS" not in t.get("reason", "")]
        if stop_losses:
