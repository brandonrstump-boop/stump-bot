...
ALL_STOCK_TICKERS  = os.getenv("STOCK_TICKERS",  "AAPL,NVDA,MSFT,TSLA,AMD,AMZN,SPY,QQQ").split(",")
ALL_CRYPTO_TICKERS = os.getenv("CRYPTO_TICKERS", "BTC,ETH,SOL,XRP,DOGE").split(",")
INTERVAL_MINS = int(os.getenv("ANALYSIS_INTERVAL_MINUTES", "30"))

def is_stock_market_open():
    now = datetime.now(timezone.utc)
    et_total_mins = (now.hour * 60 + now.minute) - (4 * 60)
    if et_total_mins < 0:
        et_total_mins += 24 * 60
    weekday = now.weekday()
    if now.hour < 4:
    ...
    market_open = 9 * 60 + 30
    market_close = 16 * 60
    return is_weekday and (market_open <= et_total_mins < market_close)
